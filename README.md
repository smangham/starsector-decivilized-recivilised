...
See that in `src`, we have `Toaster.deciv` as the package (which matches the folder path `src/Toaster/modname`).
...
1. [ ] The first step is to change the package of our code. This is an organization thing; if there are two Java files
   ...
   will be loaded.
    1. The easy way to change this is to open up `TemplateModPlugin.java`, click on the first line (`package...`),
       ...
       .
    2. When done, you shouldn't have any code in `Toaster.deciv`. If that still exists, it can be safely deleted.
    3. 
